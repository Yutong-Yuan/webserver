...
public:
    //得到线程池对象接口
    static ThreadsPool* get_instance();
    //向工作队列中添加工作
    static bool add_work(WorkData thing);
    //获取当前工作队列大小
    static int get_worksize();
private:
    //唯一的线程池对象
    static ThreadsPool *threadspool;
    static deque < WorkData >  *work_queue;
    static pthread_t *threads;
    //私有化构造函数以及拷贝构造函数（防止克隆）
    ThreadsPool();
    ThreadsPool(const ThreadsPool &t);
    ~ThreadsPool();
    static bool addwork(WorkData thing);
    static void *work (void *args);
};

pthread_t * ThreadsPool::threads=new pthread_t [threads_num];
...
ThreadsPool::ThreadsPool()
{
    ...
    {
        ret=pthread_create(&threads[i],NULL,ThreadsPool::work,NULL);
        assert(ret==0);
...
ThreadsPool * ThreadsPool::threadspool=new ThreadsPool;

ThreadsPool* ThreadsPool::get_instance()
{
    return ThreadsPool::threadspool;
}

//该函数还需要优化
bool ThreadsPool::addwork(WorkData thing)
{
    int ret;
...
bool ThreadsPool::add_work(WorkData thing)
{
    return addwork(thing);
}

int ThreadsPool::get_worksize()
{
